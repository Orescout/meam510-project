...
void Vive510::processPulse()
{
  ...
  {
    int pulsewidth = m_usFalling - m_usRising; // ST edit after ED discussion; //TODO: test out other values eg. -4 and -5, depends on circuit and amt of ambient life

    if (pulsewidth > m_sweepWidth)
    {
      if (pulsewidth > 145)
      { // pulse too long. bad pulse
#ifdef DEBUG2
...
int Vive510::status()
{
  if (micros() - m_usFalling > 500000)
  { // no pulses in long time
